...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div
            class="mx-auto flex min-h-screen w-full max-w-5xl flex-col gap-10 bg-background p-6 text-foreground md:p-10"
        >
            <!-- Header -->
            <div
            ...
            >
                ...
                <div class="relative z-10 flex flex-col gap-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2
                                class="text-xl font-black tracking-widest text-accent uppercase"
                            >
                                Lisensi Balap Sepeda
                            </h2>
                            <p
                                class="text-[10px] font-black tracking-widest text-muted-foreground uppercase opacity-80"
                            >
                    ...
                    <div
                        v-if="profile?.license_path && profile?.uci_id"
                    ...
                    >
                        ...
                        >
                            <span
                                v-if="
                                    isLicenseValid(profile.license_valid_until)
                                "
                                class="inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-2 text-[10px] font-black tracking-widest text-emerald-500 uppercase"
                            >
                                <CheckCircle2 class="h-3 w-3" /> Lisensi Aktif
                            </span>
                            <span
                                v-else
                                class="inline-flex items-center gap-2 rounded-full border border-destructive/30 bg-destructive/10 px-4 py-2 text-[10px] font-black tracking-widest text-destructive uppercase"
                            >
                            ...
                            <button
                                ...
                                @click="
                                    showPreview(
                                        `/documents/${$page.props.auth.user.id}/license`,
                                    )
                                "
                                class="mt-2 text-left text-[10px] font-black tracking-widest text-accent uppercase hover:underline"
                            >
                    ...
                    <div
                        v-else
                        class="flex items-center justify-center rounded-2xl border border-dashed border-accent/20 bg-background/50 p-10"
                    >
                        <p
                            class="text-sm font-bold text-muted-foreground italic"
                        >
                            Belum ada lisensi yang ditautkan ke akun Anda. Harap
                            hubungi pelatih atau manajemen.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <form
                ...
                class="flex flex-col gap-8 rounded-[2.5rem] border border-border bg-card p-8 shadow-xl"
            >
                <div>
                    <h3
                    ...
                    <p class="mt-1 text-sm font-medium text-muted-foreground">
                        Unggah salinan dokumen resmi Anda untuk keperluan
                        verifikasi pendaftaran kompetisi. (Maks 5MB per file)
                    </p>
                </div>
                ...
                <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                    ...
                    <div class="flex flex-col gap-3">
                        <Label
                            class="flex items-center gap-2 text-xs font-black text-foreground uppercase"
                        >
                            <UserCircle class="h-4 w-4 text-orange-500" /> Pas
                            Foto Pribadi
                        </Label>
                        <div
                            class="flex flex-col gap-2 rounded-2xl border border-dashed border-border bg-muted/20 p-6 transition-all hover:bg-muted/40"
                        >
                            <input
                                type="file"
                                @change="
                                    (e) => handleFileUpload(e, 'profile_photo')
                                "
                                accept="image/*"
                            ...
                            >
                                ...
                                <button
                                    ...
                                    @click="
                                        showPreview(
                                            `/documents/${$page.props.auth.user.id}/profile_photo`,
                                        )
                                    "
                                    class="text-[10px] font-black text-accent uppercase hover:underline"
                                >
                                    Lihat
                                </button>
                            </div>
                    ...
                    <div class="flex flex-col gap-3">
                        <Label
                            class="flex items-center gap-2 text-xs font-black text-foreground uppercase"
                        >
                            <CreditCard class="h-4 w-4 text-blue-500" /> Kartu
                            Tanda Penduduk (KTP)
                        </Label>
                        <div
                            class="flex flex-col gap-2 rounded-2xl border border-dashed border-border bg-muted/20 p-6 transition-all hover:bg-muted/40"
                        >
                            <input
                                ...
                                class="text-xs font-medium file:mr-4 file:rounded-full file:border-none file:bg-blue-500/10 file:px-4 file:py-2 file:text-[10px] file:font-black file:text-blue-500 file:uppercase hover:file:bg-blue-500/20"
                            />
                            <div
                                v-if="profile?.id_card_path"
                                class="mt-2 flex items-center justify-between rounded-xl border border-border bg-card p-3 shadow-sm"
                            >
                                <div class="flex items-center gap-2">
                                    <CheckCircle2
                                ...
                                <button
                                    ...
                                    @click="
                                        showPreview(
                                            `/documents/${$page.props.auth.user.id}/id_card`,
                                        )
                                    "
                                    class="text-[10px] font-black text-accent uppercase hover:underline"
                                >
                    ...
                    <div class="flex flex-col gap-3">
                        <Label
                            class="flex items-center gap-2 text-xs font-black text-foreground uppercase"
                        >
                            <Users class="h-4 w-4 text-emerald-500" /> Kartu
                            Keluarga (KK)
                        </Label>
                        <div
                            class="flex flex-col gap-2 rounded-2xl border border-dashed border-border bg-muted/20 p-6 transition-all hover:bg-muted/40"
                        >
                            <input
                                type="file"
                                @change="
                                    (e) => handleFileUpload(e, 'family_card')
                                "
                                accept=".pdf,.jpg,.jpeg,.png"
                                class="text-xs font-medium file:mr-4 file:rounded-full file:border-none file:bg-emerald-500/10 file:px-4 file:py-2 file:text-[10px] file:font-black file:text-emerald-500 file:uppercase hover:file:bg-emerald-500/20"
                            />
                            <div
                                v-if="profile?.family_card_path"
                                class="mt-2 flex items-center justify-between rounded-xl border border-border bg-card p-3 shadow-sm"
                            >
                                <div class="flex items-center gap-2">
                                    <CheckCircle2
                                ...
                                <button
                                    ...
                                    @click="
                                        showPreview(
                                            `/documents/${$page.props.auth.user.id}/family_card`,
                                        )
                                    "
                                    class="text-[10px] font-black text-accent uppercase hover:underline"
                                >
                        ...
                        <p
                            v-if="form.errors.family_card"
                            class="text-[10px] font-black text-destructive uppercase"
                        >
                            {{ form.errors.family_card }}
                        </p>
                    </div>
                    ...
                    <div class="flex flex-col gap-3">
                        <Label
                            class="flex items-center gap-2 text-xs font-black text-foreground uppercase"
                        >
                            <FileImage class="h-4 w-4 text-purple-500" /> Akte
                            Kelahiran
                        </Label>
                        <div
                            class="flex flex-col gap-2 rounded-2xl border border-dashed border-border bg-muted/20 p-6 transition-all hover:bg-muted/40"
                        >
                            <input
                                ...
                                "
                                accept=".pdf,.jpg,.jpeg,.png"
                                class="text-xs font-medium file:mr-4 file:rounded-full file:border-none file:bg-purple-500/10 file:px-4 file:py-2 file:text-[10px] file:font-black file:text-purple-500 file:uppercase hover:file:bg-purple-500/20"
                            />
                            <div
                                v-if="profile?.birth_certificate_path"
                                class="mt-2 flex items-center justify-between rounded-xl border border-border bg-card p-3 shadow-sm"
                            >
                                <div class="flex items-center gap-2">
                                    <CheckCircle2
                                ...
                                <button
                                    ...
                                    @click="
                                        showPreview(
                                            `/documents/${$page.props.auth.user.id}/birth_certificate`,
                                        )
                                    "
                                    class="text-[10px] font-black text-accent uppercase hover:underline"
                                >
                        ...
                        <p
                            v-if="form.errors.birth_certificate"
                            class="text-[10px] font-black text-destructive uppercase"
                        >
