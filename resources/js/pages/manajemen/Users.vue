...
const submit = () => {
    if (isEditing.value && editingUserId.value) {
        form.patch(route('manajemen.pengguna.update', editingUserId.value), {
            onSuccess: () => {
        ...
        form.post(route('manajemen.pengguna.store'), {
            onSuccess: () => {
                showModal.value = false;
                form.reset();
...
const deleteUser = async (id: number) => {
    if (
        ...
        router.delete(route('manajemen.pengguna.destroy', id), {
...
const route = (name: string, params?: any) => {
    if (name === 'manajemen.pengguna.store') {
        return '/manajemen/users';
    }

    if (name === 'manajemen.pengguna.update') {
        return `/manajemen/users/${params}`;
    }

    if (name === 'manajemen.pengguna.destroy') {
        return `/manajemen/users/${params}`;
    }
